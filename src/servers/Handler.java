...
public class Handler implements HttpHandler {
    ...
    private void handleGetPrioritized(HttpExchange exchange) throws IOException {
        Collection<Task> prioritized = manager.getPriorityTasks();
    ...
    private void handleGetById(HttpExchange exchange, String[] urlParts) throws IOException {
        ...
        } else if (urlParts.length == 4 && urlParts[2].equals("subtask") && urlParts[3].equals("epic")) {
            try {
                writeResponse(exchange, gson.toJson(manager.getAllSubtaskByEpic(id)), 200);
            } catch (NoSuchElementException e) {
                writeResponse(exchange, e.getMessage(), 404);
            }
        } else {
